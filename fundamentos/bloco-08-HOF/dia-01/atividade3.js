...
const comparandoNumeros = (myNumber, number) => myNumber === number;
const resultadoLoteria = (myNumber, callback) => {
    const number = Math.floor((Math.random() * 5) + 1);
    return callback(myNumber, number) ? 'Parabéns!' : 'Tente denovo!';
   
};
console.log(resultadoLoteria(5, comparandoNumeros));
